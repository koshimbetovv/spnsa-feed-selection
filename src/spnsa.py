...
# Helper to safely compute eigenvector centrality
...
def safe_add_path(R, ego_net, u, v, dest_key=None, storage_dict=None, weight=None):
    try:
...
def spnsa(G, AC, radius=1, weight=None, verbose=True):
    """
    ...
    # Prepare results
    paths_by_ego = {}
    # We'll build the resulting graph R by adding edges from paths
    if G.is_directed():
        R = nx.DiGraph()
    else:
        R = nx.Graph()  

    iterable = tqdm(AC, desc="Processing feed nodes") if verbose else AC
    ...
    for ego in iterable:
        ...
        # 1) ego -> MM and ego -> MI
        if MM is not None:
            pmm = safe_add_path(R, ego_net, ego, MM, dest_key='MM', storage_dict=ego_paths['ego_to_centers'], weight=weight)
        else:
            ego_paths['ego_to_centers']['MM'] = None

        if MI is not None:
            pmi = safe_add_path(R, ego_net, ego, MI, dest_key='MI', storage_dict=ego_paths['ego_to_centers'], weight=weight)
        else:
            ego_paths['ego_to_centers']['MI'] = None

        # 2) ego -> each OC
        for oc in OC:
            poc = safe_add_path(R, ego_net, ego, oc, dest_key=oc, storage_dict=ego_paths['ego_to_OC'], weight=weight)

        # 3) each OC -> centers (MM, MI)
        for oc in OC:
            if MM is not None:
                ego_paths['OC_to_centers'][oc]['to_MM'] = safe_add_path(R, ego_net, oc, MM, storage_dict=None, weight=weight)
            if MI is not None:
                ego_paths['OC_to_centers'][oc]['to_MI'] = safe_add_path(R, ego_net, oc, MI, storage_dict=None, weight=weight)
